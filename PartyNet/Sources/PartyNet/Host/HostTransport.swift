    ...
    do {
      ...
      let udpTask = Task { [udp, transport] in
        do {
          ...
          transport.eventContinuation.yield(
            .failure("UDP listener failed: \(error.localizedDescription)"))
        }
      }
      ...
      let tcpTask = Task { [tcp, transport] in
        do {
          ...
          transport.eventContinuation.yield(
            .failure("TCP listener failed: \(error.localizedDescription)"))
        }
      }
  ...
  func respond(to connectionID: UUID, with decision: HandshakeDecision) async -> Bool {
    ...
    do {
      ...
      if case .accept(let welcome) = decision {
        ...
        tokenToConnection.removeValue(forKey: welcome.sessionToken)
        lastAcknowledgmentAt.removeValue(forKey: welcome.sessionToken)
